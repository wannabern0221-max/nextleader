(async () => {
  ...
  function roleOptions(selected, editable = true) {
    const source = editable ? assignableRoles() : roles;
    return source.map(value => `<option value="${value}" ${value === selected ? 'selected' : ''}>${escapeHtml(roleLabels[value])}</option>`).join('');
  }
  function deptOptions(selected) {
    return ['정책국','정책1부','정책2부'].map(value => `<option ${value === selected ? 'selected' : ''}>${value}</option>`).join('');
  }
  ...
  async function approve(row) {
    ...
    const args = { p_target_user_id: row.dataset.id, p_new_system_role: row.querySelector('[data-role]').value, p_new_position: row.querySelector('[data-position]').value, p_new_department: row.querySelector('[data-department]').value };
  ...
  async function saveRole(row) {
    const { error } = await client.rpc('update_leader_assignment', { p_target_user_id: row.dataset.id, p_new_system_role: row.querySelector('[data-member-role]').value, p_new_position: row.querySelector('[data-member-position]').value, p_new_department: row.querySelector('[data-member-department]').value });
    if (error) return show(error.message, 'error');
