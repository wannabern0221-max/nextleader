(async()=>{
  ...
  try{
    ...
    root.innerHTML=items.map(item=>`<article class="news-card"><div><span class="badge">${esc(item.category||'간호·정책')}</span><time>${item.publishedAt?new Date(item.publishedAt).toLocaleDateString('ko-KR'):''}</time></div><h2><a href="${esc(item.link)}" target="_blank" rel="noopener noreferrer">${esc(item.title)}</a></h2><p>${esc(item.source||'외부 언론·기관')}</p><a class="more-link" href="${esc(item.link)}" target="_blank" rel="noopener noreferrer">원문 보기 →</a></article>`).join('');
