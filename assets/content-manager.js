(async () => {
  ...
  try{const{data:s}=await client.auth.getSession();if(!s.session)return location.replace('login.html');const{data,error}=await client.rpc('get_my_access');if(error)throw error;access=data;if(access?.approval_status!=='approved')return location.replace('dashboard.html');if(!['content_write_notice','content_write_card','content_write_policy','content_approve'].some(has))return location.replace('dashboard.html');applyCategoryPermissions();await load();}catch(error){show(error.message||'콘텐츠 관리 화면을 불러오지 못했습니다.','error');}

  function applyCategoryPermissions(){[...form.category.options].forEach(opt=>{opt.disabled=!(has(`content_write_${opt.value}`)||has('content_approve'));});const first=[...form.category.options].find(o=>!o.disabled);if(first)form.category.value=first.value;}
  ...
  function render(){if(!rows.length){list.innerHTML='<div class="empty-state">작성한 콘텐츠가 없습니다.</div>';return;}list.innerHTML=rows.map(r=>`<article class="manage-content-card" data-id="${r.id}"><div><span class="badge">${labels[r.category]}</span><span class="status-pill ${r.status==='published'?'approved':r.status==='rejected'?'rejected':'pending'}">${statusLabels[r.status]}</span></div><h3>${escapeHtml(r.title)}</h3><p>${escapeHtml(r.summary||'요약 없음')}</p><small>작성: ${escapeHtml(r.author_name)} 리더 · ${new Date(r.updated_at).toLocaleString('ko-KR')}</small>${r.review_note?`<div class="review-note">반려 사유: ${escapeHtml(r.review_note)}</div>`:''}<div class="action-group"><button class="action-btn" data-edit>열기</button>${['draft','rejected'].includes(r.status)?'<button class="action-btn save" data-submit>승인 요청</button>':''}${has('content_approve')&&r.status!=='published'?'<button class="action-btn approve" data-publish>게시 승인</button>':''}${has('content_approve')&&r.status==='review'?'<button class="action-btn reject" data-reject>반려</button>':''}${has('content_approve')&&r.status==='published'?'<button class="action-btn reject" data-hide>숨김</button>':''}</div></article>`).join('');
